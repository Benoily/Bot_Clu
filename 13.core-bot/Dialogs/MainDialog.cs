    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> ActStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            switch (cluResult.GetTopIntent().intent)
            {
                case FlightBooking.Intent.BookFlight:
                    ...
                    {
                        Destination = cluResult.Entities.GetDst_city(),
                        Origin = cluResult.Entities.GetOr_city(),
                        TravelDate = cluResult.Entities.GetStr_date(),
						end_travel_date = cluResult.Entities.GetEnd_date(),
						BBudget = cluResult.Entities.GetBudget(),
                    };
        ...
        private async Task<DialogTurnResult> FinalStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (stepContext.Result is BookingDetails result)
            {
				...
				//var timProperty = new TimexProperty(result.end_travel_date);
                //var travelDateMsg = timProperty.ToNaturalLanguage(DateTime.Now);
                var messageText = $"I have you booked to {result.Destination} from {result.Origin} on {result.TravelDate} to the {result.end_travel_date} for a budget of {result.BBudget}";
